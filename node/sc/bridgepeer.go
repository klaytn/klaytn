...
type BridgePeer interface {
	...
	// SendServiceChainInvalidTxResponse sends a response that contains invalid txs from child chain.
	SendServiceChainInvalidTxResponse(txHashes []common.Hash) error
}
...
func (p *baseBridgePeer) SendServiceChainInvalidTxResponse(txHashes []common.Hash) error {
	return p2p.Send(p.rw, ServiceChainInvalidTxResponseMsg, txHashes)
}
