...
func (sbh *SubBridgeHandler) broadcastServiceChainTx() {
	...
	var txs, valueTransferTxs types.Transactions
	...
	for _, tx := range allPendingTxs {
		if tx.Type().IsChainDataAnchoring() {
			...
			txs = append(txs, tx)
			valueTransferTxs = append(valueTransferTxs, tx)
		}
	}
	...
	for _, tx := range valueTransferTxs { // Remove value transfer txs once they are sent to parent bridge node and delegate resending behavior to value transfer recovery
		sbh.removeBridgeTx(tx)
	}
}
