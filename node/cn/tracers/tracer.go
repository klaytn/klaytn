...
func (jst *Tracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost, cc uint64, scope *vm.ScopeContext, depth int, err error) {
	if !jst.traceSteps {
		return
	}
	if jst.err != nil {
...
func (jst *Tracer) CaptureFault(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost, cc uint64, scope *vm.ScopeContext, depth int, err error) {
	if jst.err == nil {
