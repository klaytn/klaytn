...
// If not reverted, returns an empty string "".
{
    ...
    getLastAddress: function(callstack) {
        if(callstack.calls.length == 0)
            return callstack.to;
        return getLastAddress(callstack.calls[callstack.calls.length-1]);
    },
    ...
    result: function(ctx, db) {
        ...
        if (ctx.error == "evm: execution reverted") {
            ...
            var revertedContractAddress = ctx.to;
            if (len > 0) {
                revertedContractAddress = getLastAddress(this.callstack[len-1]);
            }
            result = {"revertedContractAddress": revertedContractAddress, "revertMessage": this.revertString};
        }
        return result;
    }
}
