...
func TestSubstituteTxByGasPrice(t *testing.T) {
	...
	if result, _ := txList.Add(oldTx, DefaultTxPoolConfig.PriceBump, false); !result {
		t.Error("it cannot add tx in tx list.")
	}

	result, replaced := txList.Add(newTx, DefaultTxPoolConfig.PriceBump, false)
	if !result {
...
func TestSubstituteTransactionAbort(t *testing.T) {
	...
	if result, replaced := txList.Add(newTx, DefaultTxPoolConfig.PriceBump, false); result || replaced != nil {
