...
func Kip103Transition(pendingState *state.StateDB, pendingHeader *types.Header, chain backends.BlockChainForCaller) {
	...
	c := NewKip103ContractCaller(pendingState, chain, pendingHeader)
	result, err := RebalanceTreasury(pendingState, chain, pendingHeader, c)
	if err != nil {
...
func RebalanceTreasury(state *state.StateDB, chain backends.BlockChainForCaller, header *types.Header, c bind.ContractCaller) (*kip103result, error) {
	...
	if blockNum, err := caller.RebalanceBlockNumber(nil); err != nil || blockNum.Cmp(header.Number) != 0 {
		return result, ErrKip103IncorrectBlock
	}
	...
	if status, err := caller.Status(nil); err != nil || status != 2 {
		return result, ErrKip103BadStatus
	}
	...
	if totalRetiredAmount.Cmp(totalNewbieAmount) < 0 {
		return result, ErrKip103NotEnoughBalance
	}
