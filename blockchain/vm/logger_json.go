...
func (l *JSONLogger) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost, cc uint64, scope *ScopeContext, depth int, err error) {
	memory := scope.Memory
	stack := scope.Stack
	log := StructLog{
		Pc:            pc,
...
func (l *JSONLogger) CaptureFault(env *EVM, pc uint64, op OpCode, gas, cost, cc uint64, scope *ScopeContext, depth int, err error) {
	l.CaptureState(env, pc, op, gas, cost, cc, scope, depth, err)
}
