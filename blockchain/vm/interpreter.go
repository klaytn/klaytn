...
func (in *EVMInterpreter) Run(contract *Contract, input []byte) (ret []byte, err error) {
	...
	if in.cfg.Debug {
		defer func() {
			if err != nil {
				if !logged {
					in.cfg.Tracer.CaptureState(in.evm, pcCopy, op, gasCopy, cost, ccCostCopy, cc, callContext, in.evm.depth, err)
				} else {
					in.cfg.Tracer.CaptureFault(in.evm, pcCopy, op, gasCopy, cost, ccCostCopy, cc, callContext, in.evm.depth, err)
				}
			}
		}()
	}
	...
	for atomic.LoadInt32(&in.evm.abort) == 0 {
		...
		if in.cfg.Debug {
			...
			logged, pcCopy, gasCopy, ccCostCopy = false, pc, contract.Gas, in.evm.Config.ComputationCostLimit-in.evm.opcodeComputationCostSum
		}
		...
		if in.cfg.Debug {
			in.cfg.Tracer.CaptureState(in.evm, pc, op, gasCopy, cost, ccCostCopy, cc, callContext, in.evm.depth, err)
			logged = true
		}
