...
func benchInsertChain(b *testing.B, dbType database.DBType, gen func(int, *BlockGen)) {
	...
	chainman, _ := NewBlockChain(db, nil, gspec.Config, gxhash.NewFaker(), vm.Config{}, false)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchReadChain(b *testing.B, full bool, databaseType database.DBType, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, params.TestChainConfig, gxhash.NewFaker(), vm.Config{}, false)
		if err != nil {
