...
// blockHashBroadcasterFn is a callback type for broadcasting a blockHash to connected peers.
type blockHashBroadcasterFn func(block *types.Block)
...
type Fetcher struct {
	...
	broadcastBlockHash blockHashBroadcasterFn // Broadcasts a blockHash to connected peers
...
func New(getBlock blockRetrievalFn, verifyHeader headerVerifierFn, broadcastBlock blockBroadcasterFn, broadcastBlockHash blockHashBroadcasterFn, chainHeight chainHeightFn, insertChain chainInsertFn, dropPeer peerDropFn) *Fetcher {
	return &Fetcher{
		...
		broadcastBlockHash: broadcastBlockHash,
		chainHeight:        chainHeight,
		insertChain:        insertChain,
		dropPeer:           dropPeer,
		insertTasks:        make(chan insertTask, numInsertTasks),
	}
}
...
func (f *Fetcher) FilterBodies(peer string, transactions [][]*types.Transaction, time time.Time) [][]*types.Transaction {
	logger.Trace("Filtering bodies", "peer", peer, "bodies", len(transactions))
...
func (f *Fetcher) loop() {
	...
	for {
		...
		for !f.queue.Empty() {
			...
			hash := op.block.Hash()
			if number <= height || f.getBlock(hash) != nil {
				f.forgetBlock(hash)
				continue
			}
		...
		case notification := <-f.notify:
			...
			if notification.number > 0 {
				if dist := int64(notification.number) - int64(f.chainHeight()); dist <= 0 || dist > maxQueueDist {
			...
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && announce.origin == task.peer && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if header.Number.Uint64() != announce.number {
						logger.Trace("Invalid block number fetched", "peer", announce.origin, "hash", header.Hash(), "announced", announce.number, "provided", header.Number)
					...
					if f.getBlock(hash) == nil {
						...
						logger.Trace("Block already imported, discarding header", "peer", announce.origin, "number", header.Number, "hash", header.Hash())
						f.forgetHash(hash)
					}
				} else {
...
func (f *Fetcher) enqueue(peer string, block *types.Block) {
	...
	if count > blockLimit {
		logger.Debug("Discarded propagated block, exceeded allowance", "peer", peer, "number", block.Number(), "hash", hash, "limit", blockLimit)
	...
	if dist := int64(block.NumberU64()) - int64(f.chainHeight()); dist <= 0 || dist > maxQueueDist {
		logger.Trace("Discarded propagated block, too far away", "peer", peer, "number", block.Number(), "hash", hash, "distance", dist)
...
func (f *Fetcher) insertWork(peer string, block *types.Block) {
	hash := block.Hash()
