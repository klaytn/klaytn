...
func (d *Downloader) RegisterPeer(id string, version int, peer Peer) error {
	localLogger := logger.NewWith("peer", id)
	localLogger.Trace("Registering sync peer")
	if err := d.peers.Register(newPeerConnection(id, version, peer, localLogger)); err != nil {
		localLogger.Error("Failed to register sync peer", "err", err)
		return err
	}
...
func (d *Downloader) UnregisterPeer(id string) error {
	...
	localLogger := logger.NewWith("peer", id)
	localLogger.Trace("Unregistering sync peer")
	if err := d.peers.Unregister(id); err != nil {
		localLogger.Error("Failed to unregister sync peer", "err", err)
		return err
	}
	d.queue.Revoke(id)
...
func (d *Downloader) spawnSync(fetchers []func() error, peerID string) error {
...
func (d *Downloader) findAncestor(p *peerConnection, height uint64) (uint64, error) {
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
				p.logger.Error("Empty head header set")
			...
			for i := len(headers) - 1; i >= 0; i-- {
				...
				if (mode == FullSync && d.blockchain.HasBlock(headers[i].Hash(), headers[i].Number.Uint64())) || (mode != FullSync && d.lightchain.HasHeader(headers[i].Hash(), headers[i].Number.Uint64())) {
					...
					if number > height && i == limit-1 {
						p.logger.Error("Lied about chain head", "reported", height, "found", number)
	...
	if !common.EmptyHash(hash) {
		if int64(number) <= floor {
			p.logger.Error("Ancestor below allowance", "number", number, "hash", hash, "allowance", floor)
	...
	for start+1 < end {
		...
		for arrived := false; !arrived; {
			...
			case packer := <-d.headerCh:
				...
				if (mode == FullSync && !d.blockchain.HasBlock(headers[0].Hash(), headers[0].Number.Uint64())) || (mode != FullSync && !d.lightchain.HasHeader(headers[0].Hash(), headers[0].Number.Uint64())) {
					end = check
					break
				}
		...
		case packet := <-d.headerCh:
			...
			if pivoting {
				if packet.Items() == 2 {
					...
					if have, want := headers[0].Number.Uint64(), pivot+uint64(fsMinFullBlocks); have != want {
						logger.Warn("Peer sent invalid next pivot", "have", have, "want", want)
					...
					if have, want := headers[1].Number.Uint64(), pivot+2*uint64(fsMinFullBlocks)-8; have != want {
						logger.Warn("Peer sent invalid pivot confirmer", "have", have, "want", want)
						return fmt.Errorf("%w: next pivot confirmer number %d != requested %d", errInvalidChain, have, want)
					}
					logger.Warn("Pivot seemingly stale, moving", "old", pivot, "new", headers[0].Number)
			...
			if packet.Items() == 0 {
				...
				if atomic.LoadInt32(&d.committed) == 0 && pivot <= from {
					p.logger.Debug("No headers, waiting for pivot commit")
					select {
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*types.Header) ([]*types.Header, int, error) {
	logger.Debug("Filling up skeleton", "from", from)
		...
		case <-update:
			...
			if pending() == 0 {
				if !inFlight() && finished {
					logger.Debug("Data fetching completed", "type", kind)
					return nil
				}
				break
			}
...
func (d *Downloader) processHeaders(origin uint64, td *big.Int) error {
	logger.Debug("Processing headers", "origin", origin, "td", td)
	defer func(start time.Time) {
		logger.Debug("Processing headers terminated", "origin", origin, "td", td, "elapsed", time.Since(start))
	}(time.Now())
	...
	defer func() {
		if len(rollback) > 0 {
			...
			logger.Info("Rolled back headers", "count", len(hashes),
			...
			for len(headers) > 0 {
				...
				if mode == SnapSync || mode == FastSync || mode == LightSync {
					...
					if n, err := d.lightchain.InsertHeaderChain(chunk, frequency); err != nil {
						...
						logger.Debug("Invalid header encountered", "number", chunk[n].Number, "hash", chunk[n].Hash(), "parent", chunk[n].ParentHash, "err", err)
						return fmt.Errorf("%w: %v", errInvalidChain, err)
					}
...
func (d *Downloader) processFastSyncContent() error {
	...
	for {
		...
		if atomic.LoadInt32(&d.committed) == 0 {
			...
			// new pivot.
			if height := latest.Number.Uint64(); height >= pivot.Number.Uint64()+2*uint64(fsMinFullBlocks) {
				logger.Warn("Pivot became stale, moving", "old", pivot.Number.Uint64(), "new", height-uint64(fsMinFullBlocks))
				pivot = results[len(results)-1-fsMinFullBlocks].Header // must exist as lower old pivot is uncommitted
...
func (d *Downloader) qosTuner() {
	for {
		...
		logger.Debug("Recalculated downloader QoS values", "rtt", rtt, "confidence", float64(conf)/1000000.0, "ttl", d.requestTTL())
		select {
		case <-d.quitCh:
			return
		case <-time.After(rtt):
...
func (d *Downloader) qosReduceConfidence() {
	...
	logger.Debug("Relaxed downloader QoS values", "rtt", rtt, "confidence", float64(conf)/1000000.0, "ttl", d.requestTTL())
}
