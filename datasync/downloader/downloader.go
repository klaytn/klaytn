...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	logger.Debug("Synchronising with the network", "peer", p.id, "klay", p.version, "head", hash, "td", td, "mode", d.mode)
			...
			for len(headers) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {
					...
					if n, err := d.lightchain.InsertHeaderChain(chunk, frequency); err != nil {
						...
						logger.Debug("Invalid header encountered", "number", chunk[n].Number, "hash", chunk[n].Hash(), "err", err)
						return errInvalidChain
					}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		logger.Error("Downloaded item processing failed", "number", results[index].Header.Number, "hash", results[index].Header.Hash(), "err", err)
		return errInvalidChain
	}
	return nil
}
	...
	for {
		...
		if atomic.LoadInt32(&d.committed) == 0 {
			...
			if height := latest.Number.Uint64(); height > pivot+2*uint64(fsMinFullBlocks) {
				logger.Debug("Pivot became stale, moving", "old", pivot, "new", height-uint64(fsMinFullBlocks))
				pivot = height - uint64(fsMinFullBlocks)
			}
		}
