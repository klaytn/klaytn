...
func setExpectation(m *mocks.MockBlockchainAPI, contract *common.Address, data, result []byte) {
	...
	m.EXPECT().Call(gomock.Any(), gomock.Eq(arg), gomock.Eq(rpc.LatestBlockNumber)).Return(result, nil).Times(1)
}
...
func (s *SuiteContractCaller) TestContractCaller_SupportsInterface_NoContractCode() {
