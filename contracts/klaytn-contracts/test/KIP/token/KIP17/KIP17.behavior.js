...
function shouldBehaveLikeKIP17 (
  ...
  shouldSupportInterfaces(['ERC165', 'KIP17']);
  ...
  context('with minted tokens', function () {
    ...
    describe('transfers', function () {
      ...
      const shouldTransferTokensByUsers = function (transferFunction) {
        ...
        context('when sent to the owner', function () {
          ...
          it('clears the approval for the token ID', async function () {
            expect(
              await this.token.getApproved(tokenId),
          ...
          it('keeps same tokens by index', async function () {
            ...
            const tokensListed = await Promise.all(
              [0, 1].map((i) =>
                this.token.tokenOfOwnerByIndex(owner, i),
              ),
            );
            expect(
      ...
      describe('via safeTransferFrom', function () {
        ...
        const shouldTransferSafely = function (transferFun, data) {
          ...
          describe('to a valid receiver contract', function () {
            beforeEach(async function () {
              this.receiver = await KIP17ReceiverMock.new(
                RECEIVER_MAGIC_VALUE,
                Error.None,
              );
              this.ERC721receiver = await ERC721ReceiverMock.new(
                ERC721_RECEIVER_MAGIC_VALUE,
                Error.None);
            ...
            it('should call onERC721Received', async function () {
              const receipt = await transferFun.call(
                this, owner, this.ERC721receiver.address, tokenId, { from: owner });
            ...
            it('calls onKIP17Received', async function () {
              const receipt = await transferFun.call(
                this,
                owner,
            ...
            it('should call onERC721Received from approved', async function () {
              const receipt = await transferFun.call(
                this, owner, this.ERC721receiver.address, tokenId, { from: approved });
            ...
            it('calls onKIP17Received from approved', async function () {
              const receipt = await transferFun.call(
                this,
                owner,
                this.receiver.address,
        ...
        describe('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            const invalidReceiver = await KIP17ReceiverMock.new(
              '0x42',
              Error.None,
            );
            await expectRevert(
        ...
        describe('to a receiver contract that reverts with message', function () {
          it('reverts', async function () {
            const revertingReceiver = await KIP17ReceiverMock.new(
              RECEIVER_MAGIC_VALUE,
    ...
    describe('safe mint', function () {
      ...
      describe('via safeMint', function () {
        // regular minting is tested in KIP17Mintable.test.js and others
        it('calls onKIP17Received — with data', async function () {
          this.receiver = await KIP17ReceiverMock.new(
            RECEIVER_MAGIC_VALUE,
            Error.None,
          );
        ...
        it('calls onKIP17Received — without data', async function () {
          this.receiver = await KIP17ReceiverMock.new(
            RECEIVER_MAGIC_VALUE,
            Error.None,
          );
        ...
        context(
          ...
          function () {
            it('reverts', async function () {
              const revertingReceiver =
                                await KIP17ReceiverMock.new(
                                  RECEIVER_MAGIC_VALUE,
                                  Error.RevertWithMessage,
                                );
              await expectRevert(
    ...
    describe('setApprovalForAll', function () {
      ...
      context('when the operator is the owner', function () {
        it('reverts', async function () {
          await expectRevert(
            this.token.setApprovalForAll(owner, true, {
              from: owner,
            }),
...
function shouldBehaveLikeKIP17Enumerable (
  errorPrefix,
  owner,
  newOwner,
  approved,
  ...
  shouldSupportInterfaces(['KIP17Enumerable']);
    ...
    describe('tokenOfOwnerByIndex', function () {
      ...
      describe('after transferring all tokens to another user', function () {
        ...
        it('returns correct token IDs for target', async function () {
          ...
          const tokensListed = await Promise.all(
            [0, 1].map((i) =>
              this.token.tokenOfOwnerByIndex(other, i),
            ),
          );
          expect(
            tokensListed.map((t) => t.toNumber()),
...
function shouldBehaveLikeKIP17Metadata (errorPrefix, name, symbol, owner) {
  shouldSupportInterfaces(['KIP17Metadata']);
