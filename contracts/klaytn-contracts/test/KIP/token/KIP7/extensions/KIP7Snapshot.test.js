...
const KIP7SnapshotMock = artifacts.require('KIP7SnapshotMock');

const { expect } = require('chai');

contract('KIP7Snapshot', function (accounts) {
  ...
  beforeEach(async function () {
    this.token = await KIP7SnapshotMock.new(
      name,
      symbol,
      initialHolder,
      initialSupply,
  ...
  describe('totalSupplyAt', function () {
    it('reverts with a snapshot id of 0', async function () {
      await expectRevert(
        this.token.totalSupplyAt(0),
        'KIP7Snapshot: id is 0',
      );
    });

    it('reverts with a not-yet-created snapshot id', async function () {
      await expectRevert(
        this.token.totalSupplyAt(1),
        'KIP7Snapshot: nonexistent id',
      );
    });
    ...
    context('with initial snapshot', function () {
      ...
      context('with no supply changes after the snapshot', function () {
        it('returns the current total supply', async function () {
          expect(
            await this.token.totalSupplyAt(this.initialSnapshotId),
        ...
        context(
          ...
          function () {
            ...
            it('snapshots return the supply before and after the changes', async function () {
              expect(
                await this.token.totalSupplyAt(
        ...
        context(
          ...
          function () {
            ...
            it('all posterior snapshots return the supply after the changes', async function () {
              ...
              for (const id of this.secondSnapshotIds) {
                expect(
                  await this.token.totalSupplyAt(id),
  ...
  describe('balanceOfAt', function () {
    it('reverts with a snapshot id of 0', async function () {
      await expectRevert(
        this.token.balanceOfAt(other, 0),
        'KIP7Snapshot: id is 0',
      );
    });
    ...
    it('reverts with a not-yet-created snapshot id', async function () {
      await expectRevert(
        this.token.balanceOfAt(other, 1),
        'KIP7Snapshot: nonexistent id',
      );
    });
    ...
    context('with initial snapshot', function () {
      ...
      context('with balance changes after the snapshot', function () {
        beforeEach(async function () {
          await this.token.transfer(recipient, new BN('10'), {
            from: initialHolder,
          });
          await this.token.mint(other, new BN('50'));
          await this.token.burn(initialHolder, new BN('20'));
          ...
          function () {
            ...
            it('all posterior snapshots return the supply after the changes', async function () {
              ...
              for (const id of this.secondSnapshotIds) {
                ...
                expect(
                  await this.token.balanceOfAt(recipient, id),
                ).to.be.bignumber.equal(
                  await this.token.balanceOf(recipient),
                );
                expect(
                  await this.token.balanceOfAt(other, id),
                ).to.be.bignumber.equal(
                  await this.token.balanceOf(other),
                );
              }
