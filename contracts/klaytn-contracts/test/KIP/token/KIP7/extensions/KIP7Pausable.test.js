...
contract('KIP7Pausable', function (accounts) {
  ...
  beforeEach(async function () {
    this.token = await KIP7PausableMock.new(
      name,
      symbol,
      holder,
      initialSupply,
  ...
  describe('pausable token', function () {
    describe('transfer', function () {
      ...
      it('reverts when trying to transfer when paused', async function () {
        ...
        await expectRevert(
          this.token.transfer(recipient, initialSupply, {
            from: holder,
          }),
    ...
    describe('transfer from', function () {
      ...
      it('allows to transfer from when unpaused', async function () {
        await this.token.transferFrom(holder, recipient, allowance, {
          from: anotherAccount,
        });
        ...
        expect(
          await this.token.balanceOf(holder),
        ).to.be.bignumber.equal(initialSupply.sub(allowance));
      });
      ...
      it('reverts when trying to transfer from when paused', async function () {
        ...
        await expectRevert(
          this.token.transferFrom(holder, recipient, allowance, {
            from: anotherAccount,
          }),
          'KIP7Pausable: token transfer while paused',
        );
    ...
    describe('burn', function () {
      ...
      it('allows to burn when unpaused', async function () {
        ...
        expect(
          await this.token.balanceOf(holder),
        ).to.be.bignumber.equal(initialSupply.sub(amount));
      });
