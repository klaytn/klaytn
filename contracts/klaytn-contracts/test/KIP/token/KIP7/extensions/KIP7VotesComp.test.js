...
const KIP7VotesCompMock = artifacts.require('KIP7VotesCompMock')
...
contract('KIP7VotesComp', function (accounts) {
    const [
        holder,
        recipient,
        holderDelegatee,
    ...
    beforeEach(async function () {
        this.token = await KIP7VotesCompMock.new(name, symbol)
    ...
    describe('set delegation', function () {
        describe('call', function () {
            it('delegation with balance', async function () {
                ...
                expect(await this.token.delegates(holder)).to.be.equal(
                    ZERO_ADDRESS
                )

                const { receipt } = await this.token.delegate(holder, {
                    from: holder,
                })
                ...
                expect(
                    await this.token.getCurrentVotes(holder)
                ...
                expect(
                    await this.token.getPriorVotes(holder, receipt.blockNumber)
                ).to.be.bignumber.equal(supply)
            })
        ...
        describe('with signature', function () {
            ...
            const delegatorAddress = web3.utils.toChecksumAddress(
                delegator.getAddressString()
            )
            const nonce = 0
            ...
            it('accept signed delegation', async function () {
                ...
                expect(
                    await this.token.delegates(delegatorAddress)
                ...
                expect(
                    await this.token.delegates(delegatorAddress)
                ).to.be.equal(delegatorAddress)

                expect(
                    await this.token.getCurrentVotes(delegatorAddress)
                ).to.be.bignumber.equal(supply)
                expect(
                    await this.token.getPriorVotes(
                        delegatorAddress,
            ...
            it('rejects bad delegatee', async function () {
                ...
                const { logs } = await this.token.delegateBySig(
                    holderDelegatee,
                    nonce,
                    MAX_UINT256,
                    v,
    ...
    describe('change delegation', function () {
        ...
        it('call', async function () {
            ...
            const { receipt } = await this.token.delegate(holderDelegatee, {
                from: holder,
            })
            expectEvent(receipt, 'DelegateChanged', {
                delegator: holder,
            ...
            expect(
                await this.token.getCurrentVotes(holderDelegatee)
            ).to.be.bignumber.equal(supply)
            expect(
                await this.token.getPriorVotes(holder, receipt.blockNumber - 1)
            ).to.be.bignumber.equal(supply)
            expect(
                await this.token.getPriorVotes(
                    holderDelegatee,
    ...
    describe('transfers', function () {
        ...
        it('no delegation', async function () {
            const { receipt } = await this.token.transfer(recipient, 1, {
                from: holder,
            })
        ...
        afterEach(async function () {
            expect(
                await this.token.getCurrentVotes(holder)
            ).to.be.bignumber.equal(this.holderVotes)
            expect(
                await this.token.getCurrentVotes(recipient)
            ).to.be.bignumber.equal(this.recipientVotes)

            // need to advance 2 blocks to see the effect of a transfer on "getPriorVotes"
            const blockNumber = await time.latestBlock()
            await time.advanceBlock()
            expect(
                await this.token.getPriorVotes(holder, blockNumber)
            ).to.be.bignumber.equal(this.holderVotes)
            expect(
                await this.token.getPriorVotes(recipient, blockNumber)
            ).to.be.bignumber.equal(this.recipientVotes)
        })
    })
    ...
    describe('Compound test suite', function () {
        ...
        describe('numCheckpoints', function () {
            it('returns the number of checkpoints for a delegate', async function () {
                ...
                expect(
                    await this.token.numCheckpoints(other1)
                ).to.be.bignumber.equal('0')

                const t1 = await this.token.delegate(other1, {
                    from: recipient,
                })
                expect(
                    await this.token.numCheckpoints(other1)
                ...
                const t2 = await this.token.transfer(other2, 10, {
                    from: recipient,
                })
                expect(
                    await this.token.numCheckpoints(other1)
                ...
                const t3 = await this.token.transfer(other2, 10, {
                    from: recipient,
                })
                expect(
                    await this.token.numCheckpoints(other1)
                ...
                const t4 = await this.token.transfer(recipient, 20, {
                    from: holder,
                })
                expect(
                    await this.token.numCheckpoints(other1)
                ...
                expect(
                    await this.token.checkpoints(other1, 0)
                ).to.be.deep.equal([t1.receipt.blockNumber.toString(), '100'])
                expect(
                    await this.token.checkpoints(other1, 1)
                ).to.be.deep.equal([t2.receipt.blockNumber.toString(), '90'])
                expect(
                    await this.token.checkpoints(other1, 2)
                ).to.be.deep.equal([t3.receipt.blockNumber.toString(), '80'])
                expect(
                    await this.token.checkpoints(other1, 3)
                ).to.be.deep.equal([t4.receipt.blockNumber.toString(), '100'])

                await time.advanceBlock()
                expect(
                    await this.token.getPriorVotes(
                        other1,
                        t1.receipt.blockNumber
                    )
                ).to.be.bignumber.equal('100')
                expect(
                    await this.token.getPriorVotes(
                        other1,
                        t2.receipt.blockNumber
                    )
                ).to.be.bignumber.equal('90')
                expect(
                    await this.token.getPriorVotes(
                        other1,
                        t3.receipt.blockNumber
                    )
                ).to.be.bignumber.equal('80')
                expect(
                    await this.token.getPriorVotes(
                        other1,
                        t4.receipt.blockNumber
                    )
                ).to.be.bignumber.equal('100')
            })
            ...
            it('does not add more than one checkpoint in a block', async function () {
                ...
                const [t1, t2, t3] = await batchInBlock([
                    () =>
                        this.token.delegate(other1, {
                            from: recipient,
                            gas: 100000,
                        }),
                    () =>
                        this.token.transfer(other2, 10, {
                            from: recipient,
                            gas: 100000,
                        }),
                    () =>
                ...
                expect(
                    await this.token.checkpoints(other1, 0)
                ).to.be.deep.equal([t1.receipt.blockNumber.toString(), '80'])
        ...
        describe('getPriorVotes', function () {
            it('reverts if block number >= current block', async function () {
                await expectRevert(
                    ...
                    'KIP7Votes: block not yet mined'
                )
            })

            it('returns 0 if there are no checkpoints', async function () {
                expect(
                    await this.token.getPriorVotes(other1, 0)
                ).to.be.bignumber.equal('0')
            })
            ...
            it('generally returns the voting balance at the appropriate checkpoint', async function () {
                ...
                await time.advanceBlock()
                await time.advanceBlock()
                const t4 = await this.token.transfer(holder, 20, {
                    from: other2,
                })
                await time.advanceBlock()
                await time.advanceBlock()
    ...
    describe('getPastTotalSupply', function () {
        ...
        it('returns 0 if there are no checkpoints', async function () {
            expect(
                await this.token.getPastTotalSupply(0)
            ).to.be.bignumber.equal('0')
        })
        ...
        it('returns the latest block if >= last checkpoint block', async function () {
            ...
            expect(
                await this.token.getPastTotalSupply(t1.receipt.blockNumber)
            ).to.be.bignumber.equal(supply)
            expect(
                await this.token.getPastTotalSupply(t1.receipt.blockNumber + 1)
            ).to.be.bignumber.equal(supply)
        })
        ...
        it('generally returns the voting balance at the appropriate checkpoint', async function () {
            ...
            expect(
                await this.token.getPastTotalSupply(t2.receipt.blockNumber)
            ).to.be.bignumber.equal('9999999999999999999999990')
            expect(
                await this.token.getPastTotalSupply(t2.receipt.blockNumber + 1)
            ).to.be.bignumber.equal('9999999999999999999999990')
            expect(
                await this.token.getPastTotalSupply(t3.receipt.blockNumber)
            ).to.be.bignumber.equal('9999999999999999999999980')
            expect(
                await this.token.getPastTotalSupply(t3.receipt.blockNumber + 1)
            ).to.be.bignumber.equal('9999999999999999999999980')
            expect(
                await this.token.getPastTotalSupply(t4.receipt.blockNumber)
            ).to.be.bignumber.equal('10000000000000000000000000')
            expect(
                await this.token.getPastTotalSupply(t4.receipt.blockNumber + 1)
            ).to.be.bignumber.equal('10000000000000000000000000')
        })
    })
})
