...
const KIP7FlashMintMock = artifacts.require('KIP7FlashMintMock')
const ERC3156FlashBorrowerMock = artifacts.require('ERC3156FlashBorrowerMock')

contract('KIP7FlashMint', function (accounts) {
    ...
    beforeEach(async function () {
        this.token = await KIP7FlashMintMock.new(
            name,
            symbol,
            initialHolder,
            initialSupply
    ...
    describe('maxFlashLoan', function () {
        it('token match', async function () {
            expect(
                await this.token.maxFlashLoan(this.token.address)
            ).to.be.bignumber.equal(MAX_UINT256.sub(initialSupply))
        })

        it('token mismatch', async function () {
            expect(
                await this.token.maxFlashLoan(ZERO_ADDRESS)
    ...
    describe('flashFee', function () {
        it('token match', async function () {
            expect(
                await this.token.flashFee(this.token.address, loanAmount)
            ).to.be.bignumber.equal('0')
        })

        it('token mismatch', async function () {
            await expectRevert(
                this.token.flashFee(ZERO_ADDRESS, loanAmount),
                'KIP7FlashMint: wrong token'
            )
        })
    })
    ...
    describe('flashLoan', function () {
        it('success', async function () {
            ...
            const { tx } = await this.token.flashLoan(
                receiver.address,
                this.token.address,
                loanAmount,
                '0x'
            ...
            expect(
                await this.token.balanceOf(receiver.address)
            ).to.be.bignumber.equal('0')
            expect(
                await this.token.allowance(receiver.address, this.token.address)
            ).to.be.bignumber.equal('0')
        })
        ...
        it('unavailable funds', async function () {
            const receiver = await ERC3156FlashBorrowerMock.new(true, true)
            const data = this.token.contract.methods
                .transfer(other, 10)
                .encodeABI()
            await expectRevert(
                this.token.flashLoan(
