...
function shouldBehaveLikeKIP7Capped (minter, [other], cap) {
  describe('capped token', function () {
    ...
    it('fails to mint if the amount exceeds the cap', async function () {
      await this.token.mint(other, cap.subn(1), { from });
      await expectRevert(
        this.token.mint(other, 2, { from }),
        'KIP7Capped: cap exceeded',
      );
    });
