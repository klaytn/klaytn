...
const KIP7PresetMinterPauser = artifacts.require('KIP7PresetMinterPauser');

contract('KIP7PresetMinterPauser', function (accounts) {
  ...
  const PAUSER_ROLE = web3.utils.soliditySha3('KIP7_PAUSER_ROLE');

  beforeEach(async function () {
    this.token = await KIP7PresetMinterPauser.new(name, symbol, {
      from: deployer,
    });
  });
  ...
  describe('pausing', function () {
    ...
    it('cannot mint while paused', async function () {
      ...
      await expectRevert(
        ...
        'KIP7Pausable: token transfer while paused',
      );
    });
    ...
    it('other accounts cannot pause', async function () {
      await expectRevert(
        ...
        'KIP7PresetMinterPauser: must have pauser role to pause',
      );
    });
    ...
    it('other accounts cannot unpause', async function () {
      ...
      await expectRevert(
        ...
        'KIP7PresetMinterPauser: must have pauser role to unpause',
      );
    });
  });
  ...
  describe('burning', function () {
    it('holders can burn their tokens', async function () {
      ...
      const receipt = await this.token.burn(amount.subn(1), {
        from: other,
      });
      expectEvent(receipt, 'Transfer', {
        from: other,
