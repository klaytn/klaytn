...
contract('TokenTimelock', function (accounts) {
  ...
  context('with token', function () {
    beforeEach(async function () {
      this.token = await KIP7Mock.new(name, symbol, beneficiary, 0); // We're not using the preminted tokens
    });
    ...
    context('once deployed', function () {
      beforeEach(async function () {
        this.releaseTime = (await time.latest()).add(
          time.duration.years(1),
        );
      ...
      it('can get state', async function () {
        ...
        expect(await this.timelock.releaseTime()).to.be.bignumber.equal(
          this.releaseTime,
        );
      });
      ...
      it('cannot be released just before time limit', async function () {
        await time.increaseTo(
          this.releaseTime.sub(time.duration.seconds(3)),
        );
        await expectRevert(
          this.timelock.release(),
          'TokenTimelock: current time is before release time',
      ...
      it('can be released just after limit', async function () {
        await time.increaseTo(
          this.releaseTime.add(time.duration.seconds(1)),
        );
      ...
      it('can be released after time limit', async function () {
        await time.increaseTo(
          this.releaseTime.add(time.duration.years(1)),
        );
        await this.timelock.release();
        expect(
          await this.token.balanceOf(beneficiary),
