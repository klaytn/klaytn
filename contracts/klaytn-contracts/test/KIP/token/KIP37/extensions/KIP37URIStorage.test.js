...
const KIP37URIStorageMock = artifacts.require('KIP37URIStorageMock');

contract(['KIP37URIStorage'], function (accounts) {
  const [holder] = accounts;

  const KIP37Uri = 'https://token.com/nfts/';
  ...
  describe('with base uri set', function () {
    beforeEach(async function () {
      this.token = await KIP37URIStorageMock.new(KIP37Uri);
      this.token.setBaseURI(baseUri);

      await this.token.mint(holder, tokenId, amount, '0x');
    });

    it('can request the token uri, returning the KIP37 uri if no token uri was set', async function () {
      const receivedTokenUri = await this.token.uri(tokenId);

      expect(receivedTokenUri).to.be.equal(KIP37Uri);
    });
  ...
  describe('with base uri set to the empty string', function () {
    beforeEach(async function () {
      this.token = await KIP37URIStorageMock.new('');
