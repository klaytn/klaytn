...
//
// This file is derived from p2p/transport.go (2023/08/30).
...
func (t *rlpxTransport) WriteMsg(msg Msg) error {
	...
	return t.conn.Write(msg.Code, t.wbuf.Bytes())
}
	...
	if t.conn != nil {
		if r, ok := err.(DiscReason); ok && r != DiscNetworkError {
			...
			if err := t.conn.SetWriteDeadline(deadline); err == nil {
				// p2p.SendItems(t.rw, p2p.discMsg, r)
...
func (t *rlpxTransport) doConnTypeHandshake(myConnType common.ConnType) (common.ConnType, error) {
	t.conn.SetDeadline(time.Now().Add(handshakeTimeout))
	return t.conn.ConnTypeHandshake(myConnType)
}
