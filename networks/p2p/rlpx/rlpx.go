...
//
// This file is derived from p2p/rlpx/rlpx.go (2018/06/04).
...
func (c *Conn) Write(code uint64, data []byte) error {
	...
	return c.handshake.writeFrame(c.conn, code, data)
}
...
func (c *Conn) ConnTypeHandshake(myConnType common.ConnType) (common.ConnType, error) {
	// myConnType validation
	werr := make(chan error, 1)
	if !myConnType.Valid() {
		return common.ConnTypeUndefined, errors.New("Connection Type is not valid")
	}

	// send myConnType
	go func() {
		_, err := c.conn.Write([]byte{byte(myConnType)})
		werr <- err
	}()

	// receive connType
	byteVal, receiveErr := bufio.NewReader(c.conn).ReadByte()

	// ensure sending is done
	sendErr := <-werr

	if receiveErr != nil {
		return common.ConnTypeUndefined, receiveErr
	}
	if sendErr != nil {
		return common.ConnTypeUndefined, sendErr
	}

	// received connType validation
	conntype := common.ConnType(int(byteVal))
	if !conntype.Valid() {
...
func (h *encHandshake) handleAuthMsg(msg *authMsgV4, prv *ecdsa.PrivateKey) error {
	...
	rpub, err := discover.NodeID(msg.InitiatorPubkey).Pubkey()
	if err != nil {
		return fmt.Errorf("bad remoteID: %#v", err)
	}
	h.initNonce = msg.Nonce[:]
	h.remote = ecies.ImportECDSAPublic(rpub)
...
// secrets are called after the handshake is completed.
