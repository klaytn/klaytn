...
//
// This file is derived from p2p/rlpx/rlpx_test.go (2018/06/04).
...
func checkMsgReadWrite(t *testing.T, p1, p2 *Conn, msgCode uint64, msgData []byte) {
	...
	go func() {
		var msg message
		msg.code, msg.data, msg.err = p1.Read()
		ch <- msg
	}()

	// Write the message.
	if err := p2.Write(msgCode, msgData); err != nil {
		t.Fatal(err)
	}
