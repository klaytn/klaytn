...
func startStop(net *Network, quit chan struct{}, nodeCount int) {
	...
	for {
		select {
		case <-quit:
			logger.Info("Terminating simulation loop")
			return
		case <-tick.C:
			id := nodes[rand.Intn(len(nodes))]
			logger.Info("stopping node", "id", id)
			if err := net.Stop(id); err != nil {
				logger.Error("error stopping node", "id", id, "err", err)
				return
			}
			...
			logger.Debug("starting node", "id", id)
			if err := net.Start(id); err != nil {
				logger.Error("error starting node", "id", id, "err", err)
				return
			}
		}
	}
...
func probabilistic(net *Network, quit chan struct{}, nodeCount int) {
	...
	for {
		...
		for i := lowid; i < highid; i++ {
			...
			logger.Debug(fmt.Sprintf("node %v shutting down", nodes[i]))
			err := net.Stop(nodes[i])
			if err != nil {
				logger.Error("Error stopping node", "node", nodes[i])
				wg.Done()
				continue
			}
			go func(id discover.NodeID) {
				...
				err := net.Start(id)
				if err != nil {
					logger.Error("Error starting node", "node", id)
				}
				wg.Done()
			}(nodes[i])
		}
...
func connectNodesInRing(net *Network, nodeCount int) ([]discover.NodeID, error) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		if err != nil {
			logger.Error("Error creating a node!", "err", err)
			return nil, err
		}
		ids[i] = node.ID()
	}
	...
	for _, id := range ids {
		if err := net.Start(id); err != nil {
			logger.Error("Error starting a node!", "err", err)
			return nil, err
		}
		logger.Debug(fmt.Sprintf("node %v starting up", id))
	}
	for i, id := range ids {
		peerID := ids[(i+1)%len(ids)]
		if err := net.Connect(id, peerID); err != nil {
			logger.Error("Error connecting a node to a peer!", "err", err)
			return nil, err
		}
	}
