...
func (c *Client) send(ctx context.Context, op *requestOp, msg interface{}) error {
	select {
	case c.reqInit <- op:
		logger.Trace("rpc client send", "msg", log.Lazy{Fn: func() string {
...
func (c *Client) reconnect(ctx context.Context) error {
	...
	if err != nil {
		logger.Trace("RPC client reconnect failed", "err", err)
		return err
	}
	select {
...
func (c *Client) dispatch(codec ServerCodec) {
	...
	for {
		...
		case err := <-c.readErr:
			logger.Debug("RPC connection read error", "err", err)
			conn.close(err, lastOp)
			reading = false

		// Reconnect:
		case newcodec := <-c.reconnected:
			logger.Debug("<-reconnected", "reading", reading, "remote", newcodec.remoteAddr())
			if reading {
