...
func (h *handler) handleImmediate(msg *jsonrpcMessage) bool {
	...
	case msg.isResponse():
		...
		logger.Trace("Handled RPC response", "reqid", idForLog{msg.ID}, "duration", time.Since(start))
		return true
	default:
		return false
	}
...
func (h *handler) handleResponse(msg *jsonrpcMessage) {
	...
	if op == nil {
		logger.Debug("Unsolicited RPC response", "reqid", idForLog{msg.ID})
		return
	}
...
func (h *handler) handleCallMsg(ctx *callProc, msg *jsonrpcMessage) *jsonrpcMessage {
	...
	case msg.isCall():
		...
		if resp.Error != nil {
			...
			logger.Debug("Served "+msg.Method, ctx...)
		} else {
			logger.Trace("Served "+msg.Method, ctx...)
		}
		return resp
	case msg.hasValidID():
		rpcErrorResponsesCounter.Inc(1)
