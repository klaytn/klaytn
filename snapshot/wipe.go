...
func wipeSnapshot(db database.DBManager, full bool) chan struct{} {
	...
	if full {
		db.DeleteSnapshotRoot()
	}
	// Wipe everything else asynchronously
	wiper := make(chan struct{}, 1)
	go func() {
		if err := wipeContent(db); err != nil {
			logger.Error("Failed to wipe state snapshot", "err", err) // Database close will trigger this
			return
		}
		close(wiper)
	}()
...
func wipeContent(db database.DBManager) error {
	...
	logger.Info("Compacting snapshot account area ")
...
func wipeKeyRange(db database.DBManager, kind string, prefix []byte, keylen int) error {
	...
	it := db.GetDatabase(database.SnapshotDB).NewIterator(prefix, nil)
	for it.Next() {
		...
		if items%10000 == 0 {
			...
			it = db.GetDatabase(database.SnapshotDB).NewIterator(prefix, seekPos)
