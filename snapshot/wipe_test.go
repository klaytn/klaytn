...
func TestWipe(t *testing.T) {
	...
	for i := 0; i < 128; i++ {
		account := randomHash()
		db.WriteAccountSnapshot(account, randomHash().Bytes())
		for j := 0; j < 1024; j++ {
			db.WriteStorageSnapshot(account, randomHash(), randomHash().Bytes())
		}
	}
	...
	for it.Next() {
		key := it.Key()
		if len(key) == len(database.SnapshotAccountPrefix)+common.HashLength {
			items++
		}
	}
	...
	if hash := db.ReadSnapshotRoot(); hash == (common.Hash{}) {
	...
	it = snapshotDB.NewIterator(nil, nil)
	defer it.Release()
