...
func loadAndParseLegacyJournal(db database.DBManager, base *diskLayer) (snapshot, journalGenerator, error) {
...
func loadAndParseJournal(db database.DBManager, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the node
...
func loadSnapshot(diskdb database.DBManager, triedb *statedb.Database, cache int, root common.Hash, recovery bool) (snapshot, error) {
	...
	if !generator.Done {
		...
		if generator.Wiping {
			logger.Info("Resuming previous snapshot wipe")
...
func (dl *diskLayer) Journal(buffer *bytes.Buffer) (common.Hash, error) {
	...
	journalProgress(dl.diskdb.GetDatabase(database.SnapshotDB), dl.genMarker, stats)
...
func (dl *diskLayer) LegacyJournal(buffer *bytes.Buffer) (common.Hash, error) {
	...
	logger.Debug("Legacy journalled disk layer", "root", dl.root)
...
func (dl *diffLayer) LegacyJournal(buffer *bytes.Buffer) (common.Hash, error) {
	...
	logger.Debug("Legacy journalled disk layer", "root", dl.root, "parent", dl.parent.Root())
	return base, nil
}
