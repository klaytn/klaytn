...
func (stat *generateStats) report() {
	...
	logger.Info("Generating trie hash from snapshot", ctx...)
}
...
func (stat *generateStats) reportDone() {
	...
	ctx = append(ctx, []interface{}{"elapsed", common.PrettyDuration(time.Since(stat.start))}...)
	logger.Info("Generated trie hash from snapshot", ctx...)
}
...
func stdGenerate(in chan (trieKV), out chan (common.Hash)) {
	t, _ := statedb.NewTrie(common.Hash{}, statedb.NewDatabase(database.NewMemoryDBManager()))
	for leaf := range in {
