...
func (b *bridge) passwdByPrompt(call jsre.Call, msg string) (goja.Value, error) {
	fmt.Fprintln(b.printer, msg)
	input, err := b.prompter.PromptPassword("Passphrase: ")
	if err != nil {
		return nil, err
	}
	return call.VM.ToValue(input), nil
}

// passwdByPromptWithDuration returns the password and the duration from a non-echoing password prompt.
func (b *bridge) passwdByPromptWithDuration(call jsre.Call, msg string) (goja.Value, goja.Value, error) {
	var (
		passwd   goja.Value
		duration = goja.Null()
		err      error
	...
	case 0:
		passwd, err = b.passwdByPrompt(call, msg)
		if err != nil {
			return nil, nil, err
		}
	case 1:
	...
	case 2:
		if call.Argument(0).ExportType().Kind() == reflect.String && isNumber(call.Argument(1)) {
			passwd = call.Argument(0)
...
// the RPC call.
func (b *bridge) UnlockParentOperator(call jsre.Call) (goja.Value, error) {
	passwd, duration, err := b.passwdByPromptWithDuration(call, "Unlock parent operator account")
	if err != nil {
		return nil, err
	}

	// Send the request to the backend and return
	response, callable := goja.AssertFunction(getJeth(call.VM).Get("unlockParentOperator"))
	if !callable {
		return nil, errors.New("jeth.unlockParentOperator is not callable")
	}
	return response(goja.Null(), passwd, duration)
}
...
// the RPC call.
func (b *bridge) UnlockChildOperator(call jsre.Call) (goja.Value, error) {
	passwd, duration, err := b.passwdByPromptWithDuration(call, "Unlock child operator account")
	if err != nil {
		return nil, err
	}

	// Send the request to the backend and return
	response, callable := goja.AssertFunction(getJeth(call.VM).Get("unlockChildOperator"))
	if !callable {
		return nil, errors.New("jeth.unlockChildOperator is not callable")
	}
	return response(goja.Null(), passwd, duration)
}
