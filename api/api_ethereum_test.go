...
func createTestData(t *testing.T, header *types.Header) (*types.Block, types.Transactions, map[common.Hash]*types.Transaction, map[common.Hash]*types.Receipt, []*types.Receipt) {
	...
	{
		dummyBlock := types.NewTestBlock(&types.Header{}, nil, nil, statedb.NewStackTrie(nil))
	...
	if header != nil {
		block = types.NewTestBlock(header, txs, receipts, statedb.NewStackTrie(nil))
	} else {
		block = types.NewTestBlock(&types.Header{Number: big.NewInt(1)}, txs, nil, statedb.NewStackTrie(nil))
	}
