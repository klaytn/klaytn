...
func (s *PublicBlockChainAPI) DoEstimateGas(ctx context.Context, b Backend, args CallArgs, gasCap *big.Int) (hexutil.Uint64, error) {
	...
	executable := func(gas uint64) bool {
		...
		_, _, _, failed, err := DoCall(ctx, b, args, rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber), vm.Config{UseOpcodeComputationCost: true}, localTxExecutionTime, gasCap)
		if err != nil || failed {
			return false
		}
		return true
