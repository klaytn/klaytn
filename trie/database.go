...
type Database struct {
	diskdb gxdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb gxdb.Database) *Database {
	return &Database{
		diskdb: diskdb,
...
func (db *Database) Commit(node common.Hash, report bool) error {
	...
	for hash, preimage := range db.preimages {
		...
		if batch.ValueSize() > gxdb.IdealBatchSize {
...
func (db *Database) commit(hash common.Hash, batch gxdb.Batch) error {
