...
type SimulatedBackend struct {
	...
	blockchain *blockchain.BlockChain // Blockchain to handle the consensus
...
// newSimulatedBackend creates a new binding backend with a genesis.
...
func (b *SimulatedBackend) callContract(ctx context.Context, call klaytn.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, uint64, bool, error) {
...
func (b *SimulatedBackend) SubscribeNewHead(ctx context.Context, ch chan<- *types.Header) (klaytn.Subscription, error) {
...
func (fb *filterBackend) SubscribeNewTxsEvent(ch chan<- blockchain.NewTxsEvent) event.Subscription {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		<-quit
		return nil
	})
