...
func New(backend Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, nodetype common.ConnType, rewardbase common.Address, TxResendUseLegacy bool, resetTimeOut time.Duration, resetFn func()) *Miner {
	miner := &Miner{
		backend:  backend,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, rewardbase, backend, mux, nodetype, TxResendUseLegacy, resetTimeOut, resetFn),
		canStart: 1,
	}
