...
import (
	...
	"groundx.xyz/go-gxplatform/gxp/fetcher"
	"math/big"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing GXPlatform peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping GXPlatform protocol")
	...
	log.Info("GXPlatform protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("GXPlatform peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("GXPlatform handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("GXPlatform peer registration failed", "err", err)
		return err
	}
...
func (pm *ProtocolManager) txBroadcastLoop() {
	for {
		...
		case event := <-pm.txsCh:
			...
			fmt.Printf("###### handler.txBroadcastLoop with NewTxEvent from tx_pool  txs[%d]\n", len(event.Txs))
