...
func newLeaf(key, val []byte, db database.DBManager) *StackTrie {
...
func newExt(key []byte, child *StackTrie, db database.DBManager) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.key = append(st.key, key...)
	st.children[0] = child
...
func (st *StackTrie) hashRec(hasher *hasher) {
	...
	if st.db != nil {
		...
		st.db.GetStateTrieDB().Put(st.val, encodedNode)
	}
}
